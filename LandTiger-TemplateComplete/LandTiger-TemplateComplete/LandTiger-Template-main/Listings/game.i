...
void DrawBlock(int riga, int colonna, uint16_t colore) {
    int x_pixel = 20 + (colonna * 12);
    int y_pixel = 20 + (riga * 12);
    int i;

    // Controlla limiti per sicurezza
    if(riga < 0 || riga >= 20 || colonna < 0 || colonna >= 10) return;

    for (i = 0; i < 12; i++) {
        LCD_DrawLine(x_pixel, y_pixel + i, x_pixel + 12 - 1, y_pixel + i, colore);
    }
    // Bordo
    LCD_DrawLine(x_pixel, y_pixel, x_pixel + 12 - 1, y_pixel, 0x0000);
    LCD_DrawLine(x_pixel, y_pixel + 12 - 1, x_pixel + 12 - 1, y_pixel + 12 - 1, 0x0000);
    LCD_DrawLine(x_pixel, y_pixel, x_pixel, y_pixel + 12 - 1, 0x0000);
    LCD_DrawLine(x_pixel + 12 - 1, y_pixel, x_pixel + 12 - 1, y_pixel + 12 - 1, 0x0000);
}

void DrawGridOutline() {
    int width_pixel = 10 * 12;
    int height_pixel = 20 * 12;
    LCD_DrawLine(20 - 1, 20 - 1, 20 - 1, 20 + height_pixel, 0xFFFF);
    LCD_DrawLine(20 + width_pixel, 20 - 1, 20 + width_pixel, 20 + height_pixel, 0xFFFF);
    LCD_DrawLine(20 - 1, 20 + height_pixel, 20 + width_pixel, 20 + height_pixel, 0xFFFF);
}
...
void DrawPiece(Tetromino p, int riga, int colonna) {
    ...
    for(i=0; i<4; i++) {
        ...
        if (absRow >= 0 && absRow < 20 && absCol >= 0 && absCol < 10) {
...
void ClearBoard(void) {
    ...
    for(i=0; i<20; i++) {
        for(j=0; j<10; j++) {
            ...
            DrawBlock(i, j, 0x0000); // Pulisce schermo (nero)
        }
    }
}
