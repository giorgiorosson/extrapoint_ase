...
void TIMER0_IRQHandler (void)
{
    ...
    {
        // Eseguiamo la logica di gioco SOLO se siamo nello stato PLAYING
        // Se siamo in PAUSA o RESET, il timer gira a vuoto (utile solo per rand seed)
        if (currentGameState == GAME_PLAYING) {

            // --- LOGICA DI MOVIMENTO VERSO IL BASSO ---

            // 1. Cancella il pezzo dalla posizione attuale
            // Nota: Usiamo DrawPiece ma con colore NERO (o funzione dedicata ClearPiece)
            // Se non hai una funzione 'ClearPiece', puoi modificare temporaneamente il colore
            // del pezzo corrente a Nero, disegnarlo, e poi rimetterlo al colore originale.
            uint16_t originalColor = currentPiece.color;
            currentPiece.color = 0x0000; // Imposta colore nero/vuoto
            DrawPiece(currentPiece, currentY, currentX);
            currentPiece.color = originalColor; // Ripristina colore originale

            // 2. Aggiorna la coordinata Y (Scende di 1 riga)
            currentY++;

            // 3. Disegna il pezzo nella nuova posizione
            DrawPiece(currentPiece, currentY, currentX);

            // NOTA: Qui manca ancora il controllo collisioni (se tocca il fondo).
            // Lo aggiungerai nello step successivo (Specifica 5 completa).
            // Per ora il pezzo scenderà all'infinito o sovrascriverà tutto.
        }
        ...
        ((LPC_TIM_TypeDef *) ((0x40000000UL) + 0x04000) )->IR = 8;
    }

  return;
}
# 81 "Source/timer/IRQ_timer.c"
