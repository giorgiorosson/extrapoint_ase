...
# 1 "C:\\Users\\giorg\\AppData\\Local\\Keil_v5\\ARM\\ARMCLANG\\bin\\..\\include\\stdbool.h" 1 3
# 7 "Source\\game.h" 2
...
void DrawBlock(int r, int c, uint16_t color); // Disegna singolo blocco (o cancella se 0x0000)
...
# 58 "C:\\Users\\giorg\\AppData\\Local\\Keil_v5\\ARM\\ARMCLANG\\bin\\..\\include\\string.h" 3
...
const Tetromino TETROMINOS[7] = {
    // I-Shape (0x7FFF)
    { { {-1, 0}, {0, 0}, {1, 0}, {2, 0} }, 0x7FFF },
    // O-Shape (0xFFE0)
    { { {0, 0}, {1, 0}, {0, 1}, {1, 1} }, 0xFFE0 },
    // T-Shape (0xF81F)
    { { {-1, 0}, {0, 0}, {1, 0}, {0, -1} }, 0xF81F },
    // J-Shape (0x001F)
    { { {-1, -1}, {-1, 0}, {0, 0}, {1, 0} }, 0x001F },
    // L-Shape (0xFD20 // Colore per L-Shape)
    { { {-1, 0}, {0, 0}, {1, 0}, {1, -1} }, 0xFD20 // Colore per L-Shape },
    // S-Shape (0x07E0)
    { { {-1, 1}, {0, 1}, {0, 0}, {1, 0} }, 0x07E0 },
    // Z-Shape (0xF800)
    { { {-1, 0}, {0, 0}, {0, 1}, {1, 1} }, 0xF800 }
};

// --- PROTOTIPI FUNZIONI PRIVATE ---
static _Bool CheckCollision(int px, int py, Tetromino p);
...
void DrawBlock(int r, int c, uint16_t color) {
    int x_pixel = 20 // Posizione X inizio griglia su LCD (Offset Sinistro) + (c * 12 // Dimensione in pixel di un singolo blocco);
    int y_pixel = 30 // Posizione Y inizio griglia su LCD (Offset Alto) + (r * 12 // Dimensione in pixel di un singolo blocco);
    int i;

    // Safety Check
    if (r < 0 || r >= 20 || c < 0 || c >= 10) return;

    for (i = 0; i < 12 // Dimensione in pixel di un singolo blocco; i++) {
        LCD_DrawLine(x_pixel, y_pixel + i, x_pixel + 12 // Dimensione in pixel di un singolo blocco - 1, y_pixel + i, color);
    }

    // Disegna bordo (nero se il blocco è pieno, o colore sfondo se cancelliamo)
    uint16_t borderColor = (color == 0x0000 // Sfondo del campo (vuoto)) ? 0x0000 // Sfondo del campo (vuoto) : 0x0000;

    LCD_DrawLine(x_pixel, y_pixel, x_pixel + 12 // Dimensione in pixel di un singolo blocco - 1, y_pixel, borderColor);
    LCD_DrawLine(x_pixel, y_pixel + 12 // Dimensione in pixel di un singolo blocco - 1, x_pixel + 12 // Dimensione in pixel di un singolo blocco - 1, y_pixel + 12 // Dimensione in pixel di un singolo blocco - 1, borderColor);
    LCD_DrawLine(x_pixel, y_pixel, x_pixel, y_pixel + 12 // Dimensione in pixel di un singolo blocco - 1, borderColor);
    LCD_DrawLine(x_pixel + 12 // Dimensione in pixel di un singolo blocco - 1, y_pixel, x_pixel + 12 // Dimensione in pixel di un singolo blocco - 1, y_pixel + 12 // Dimensione in pixel di un singolo blocco - 1, borderColor);
}

void DrawGridOutline(void) {
    int width_pix = 10 * 12 // Dimensione in pixel di un singolo blocco;
    int height_pix = 20 * 12 // Dimensione in pixel di un singolo blocco;

    // Linee bianche attorno al campo
    LCD_DrawLine(20 // Posizione X inizio griglia su LCD (Offset Sinistro) - 1, 30 // Posizione Y inizio griglia su LCD (Offset Alto) - 1, 20 // Posizione X inizio griglia su LCD (Offset Sinistro) - 1, 30 // Posizione Y inizio griglia su LCD (Offset Alto) + height_pix, 0xFFFF // Colore bordi campo);
    LCD_DrawLine(20 // Posizione X inizio griglia su LCD (Offset Sinistro) + width_pix, 30 // Posizione Y inizio griglia su LCD (Offset Alto) - 1, 20 // Posizione X inizio griglia su LCD (Offset Sinistro) + width_pix, 30 // Posizione Y inizio griglia su LCD (Offset Alto) + height_pix, 0xFFFF // Colore bordi campo);
    LCD_DrawLine(20 // Posizione X inizio griglia su LCD (Offset Sinistro) - 1, 30 // Posizione Y inizio griglia su LCD (Offset Alto) + height_pix, 20 // Posizione X inizio griglia su LCD (Offset Sinistro) + width_pix, 30 // Posizione Y inizio griglia su LCD (Offset Alto) + height_pix, 0xFFFF // Colore bordi campo);

    // Interfaccia Testuale Statica
    GUI_Text(160, 40, (uint8_t *)"NEXT", 0xFFFF, 0x0000); // Placeholder per Next Piece (opzionale)
    GUI_Text(160, 100, (uint8_t *)"SCORE", 0xFFFF, 0x0000);
    GUI_Text(160, 160, (uint8_t *)"LINES", 0xFFFF, 0x0000);
    GUI_Text(160, 220, (uint8_t *)"HI-SC", 0xFFFF, 0x0000);
}
...
void RefreshScore(void) {
    ...
    GUI_Text(160, 120, (uint8_t *)buffer, 0xFFFF, 0x0000);

    sprintf(buffer, "%03d", linesCleared);
    GUI_Text(160, 180, (uint8_t *)buffer, 0xFFFF, 0x0000);

    sprintf(buffer, "%06d", highScore);
    GUI_Text(160, 240, (uint8_t *)buffer, 0xF7DE, 0x0000);
}
...
void Game_Reset(void) {
    ...
    for (i = 0; i < 20; i++) {
        for (j = 0; j < 10; j++) {
            board[i][j] = 0;
            DrawBlock(i, j, 0x0000 // Sfondo del campo (vuoto));
        }
    }
...
static void RemoveLines(void) {
    ...
    for (r = 20 - 1; r >= 0; r--) {
        ...
        if (full) {
            ...
            for (c = 0; c < 10; c++) {
                board[0][c] = 0;
                DrawBlock(0, c, 0x0000 // Sfondo del campo (vuoto));
            }
...
void Game_Over(void) {
    ...
    UpdateHighscore();
    GUI_Text(90, 140, (uint8_t *)"GAME OVER", 0xF800, 0xFFE0);
}
